...
Stage 1 – M3AE Self-Supervised Pretraining (--pretrain_epochs)
  ...
  • x_sub is updated jointly via back-prop (model inversion, §Model Inversion)
...
class M3AE_Encoder(nn.Module):
    ...
    This is the architecture described in the M3AE paper:
...
class RegressionHead(nn.Module):
    ...
    (Paper: "a regression head (1×1×1 convolution without sigmoid)")
    """
...
class SegmentationHead(nn.Module):
    ...
    Deep supervision follows Eq. (5) of the paper:
...
def prepare_masked_input(x: torch.Tensor,
    ...
    """
    Full M3AE masking pipeline (Eq. 1-3 of the paper):
...
class M3AE_Liver(nn.Module):
    ...
    def forward_finetune(self, x: torch.Tensor,
        ...
        (consistency loss L_con, Eq. 2 of the paper).
        ...
        # Paper Eq. (2): L_con(x0, x1, x_sub) = MSE(f0, f1)
...
def train_finetune_epoch(model, loader, optimizer, scaler, epoch, args):
    ...
    Objective (Eq. 4 of paper):
        min_{f, fs, {fd}} λ * L_con(x0, x1, x_sub) + Σ_i L_seg(s_gt, xi, x_sub)

    Deep supervision adds the ½ and ¼ scale losses (Eq. 5).
    """
    model.train()
    model.is_training = True
...
def test_all_combinations(model, test_loader, args, save_dir):
    ...
    logging.info(f"  Average: ${flat.mean():.3f}\\pm{flat.std():.3f}"
                 f"({np.median(flat):.3f})^\\dagger$")
...
class LR_Scheduler:
    ...
    def __call__(self, optimizer, epoch):
        lr = self.lr * (1 - epoch / max(self.T, 1)) ** 0.9
...
def parse_args():
    ...
    p.add_argument("--pretrain_epochs", type=int, default=100,
                   help="Stage-1 M3AE pretraining epochs (paper: 600; 0 = skip)")
    p.add_argument("--num_epochs",      type=int, default=200,
                   help="Stage-2 fine-tuning epochs (paper: 300; 0 = skip)")
    p.add_argument("--batch_size",      type=int, default=2)
    p.add_argument("--lr",              type=float, default=3e-4,
                   help="Initial LR (paper: 0.0003 with cosine decay)")
    ...
    p.add_argument("--mask_ratio",     type=float, default=0.875,
                   help="Combined masking ratio (paper: 0.875)")
    p.add_argument("--patch_size",     type=int,   default=16,
                   help="3-D patch side length P (paper: 16)")
    p.add_argument("--sub_reg_weight", type=float, default=0.005,
                   help="L2 regularisation weight γ on x_sub (paper: 0.005)")
    p.add_argument("--con_weight",     type=float, default=0.1,
                   help="Self-distillation weight λ (paper: 0.1)")
