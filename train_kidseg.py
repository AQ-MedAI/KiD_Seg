...
class LiverDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        x, y = np.asarray(images, dtype=np.float32), label.astype(np.int64)
...
class DC_Seg_Liver(nn.Module):
    ...
    def forward(self, x, mask, return_logits=False):
        ...
        sep_preds = []
        ckpt = self.use_ckpt and self.training  # exact checkpointing of the 8 per-modality
        for m in range(self.num_modal):        # decoder / style / reconstruction passes (saves ~3 GB at 96x96x32 b2)
            sep_preds.append(grad_checkpoint(self.decoder_sep, *feats[m], use_reentrant=False)
                             if ckpt else self.decoder_sep(*feats[m]))

        # ── style + reconstruction — SEQUENTIAL ──
        recon_list, mu_list, sigma_list, styles_raw = [], [], [], []
        for m in range(self.num_modal):
            if ckpt:
                st = grad_checkpoint(self.style_encoders[m], x[:, m:m+1], use_reentrant=False)
                styles_raw.append(st)
                rec, mu, sig = grad_checkpoint(self.img_decoders[m], st, fuse_x4, use_reentrant=False)
            else:
    ...
    for i, (x, target, mask, mask_max, _) in enumerate(loader):
        ...
        with make_autocast(args.amp):
            ...
            if (model.use_dkd and kcl_scale > 0 and z_k is not None
                ...
                kcl_l = kcl(z_k.detach() if args.kcl_detach else z_k, target)
...
def visualize_test(model, test_loader, args, save_dir):
    ...
    for x, y_int, names in test_loader:
        ...
        for b in range(x.size(0)):
            ...
            pr = pl[b].cpu().numpy()
            _ta = (gt > 0).reshape(-1, gt.shape[2]).sum(0)  # tumour area per axial slice
            z = int(np.argmax(_ta)) if _ta.max() > 0 else t2.shape[2] // 2
...
def parse_args():
    ...
    p.add_argument("--kcl_detach", type=int, default=1, choices=[0, 1],
                   help="1 = detach the kinetic feature inside L_KCL (setting used for the "
                        "paper, default); 0 = let the KCL gradients reach the difference encoder")
...
def main():
    ...
    for epoch in range(args.num_epochs):
        ...
        avg = train_one_epoch(model, train_loader, optimizer, scaler, epoch, args,
                              ana_cl, mod_cl, agcl, kcl, ca_loss_fn)
        logging.info(f"  Train loss: {avg:.4f}  (peak GPU mem {torch.cuda.max_memory_reserved() / 2**30:.2f} GB)")
