...
Adapted from:
  ...
  with Missing Modalities (MICCAI 2025)
...
Usage:
  python train_dc_seg_a2cl_dkd_hac.py --datapath /path/to/preprocess_nii_256x32
...
nohup python -u train_dc_seg_a2cl_dkd_hac.py --variant baseline --datapath /ossfs/workspace/miccai2026-3/preprocess_nii_256x32 > /ossfs/workspace/miccai2026-3/logs/train_baseline.log 2>&1 &
tail -f /ossfs/workspace/miccai2026-3/logs/train_baseline.log

# Row 2 — + AGCL
nohup python -u train_dc_seg_a2cl_dkd_hac.py --variant agcl --datapath /ossfs/workspace/miccai2026-3/preprocess_nii_256x32 > /ossfs/workspace/miccai2026-3/logs/train_agcl.log 2>&1 &
tail -f /ossfs/workspace/miccai2026-3/logs/train_agcl.log

# Row 3 — + AGCL + DKD
nohup python -u train_dc_seg_a2cl_dkd_hac.py --variant agcl_dkd --datapath /ossfs/workspace/miccai2026-3/preprocess_nii_256x32 > /ossfs/workspace/miccai2026-3/logs/train_agcl_dkd.log 2>&1 &
tail -f /ossfs/workspace/miccai2026-3/logs/train_agcl_dkd.log

# Row 4 — + AGCL + DKD + HGF  (Full KiD-Seg)
nohup python -u train_dc_seg_a2cl_dkd_hac.py --variant full --datapath /ossfs/workspace/miccai2026-3/preprocess_nii_256x32 > /ossfs/workspace/miccai2026-3/logs/train_full.log 2>&1 &
tail -f /ossfs/workspace/miccai2026-3/logs/train_full.log

kill <PID>   # stop a background run (PID printed by nohup, or: ps aux | grep train_dc_seg)

# Per-component override (takes precedence over --variant), e.g. full minus HGF:
#   nohup python -u train_dc_seg_a2cl_dkd_hac.py --variant full --use_hgf 0 ... &
"""
import os
...
# ═══════════════════════════════════════════════════════════════════════════════
# 3b. HIERARCHICAL ATOMIC-GROUP FUSION (§3.4)
# ═══════════════════════════════════════════════════════════════════════════════
...
# ── Completeness-Aware Self-Distillation (§3.4, Eq. distill) ─────────────────
...
def train_one_epoch(model, loader, optimizer, scaler, epoch, args,
    ...
    for i, (x, target, mask, mask_max, _) in enumerate(loader):
        ...
        # (§3.4), so it is only active when HGF is enabled.
        teacher_logits = None
        ...
        with make_autocast(args.amp):
            ...
            loss = loss + alpha * (4 * recon_l + 4 * ana_l + 4 * mod_l
...
def parse_args():
    ...
    p.add_argument("--agcl_weight", type=float, default=4.0,
    ...
    # Completeness-Aware Self-Distillation (§3.4) – CORRECTED DEFAULTS
    ...
    p.add_argument("--variant", choices=["baseline", "agcl", "agcl_dkd", "full"],
                   default="full",
                   help="Ablation preset (Table 1). Overridable by "
...
def resolve_ablation(args):
    ...
    Maps the ablation presets of Table 1 onto (use_agcl, use_dkd, use_hgf) and
