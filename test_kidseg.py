...
Usage:
  python test_dc_seg_a2cl_dkd_hac.py --datapath /path/to/preprocess_nii_256x32

══════════════════════════════════════════════════════════════════════════════
TERMINAL COMMANDS — Ablation study (Table 1), one script, --variant controlled
...
conda init bash
source ~/.bashrc
cd /ossfs/workspace/miccai2026-3
export CUDA_VISIBLE_DEVICES=0
conda activate brain_dinov2
clear
mkdir -p /ossfs/workspace/miccai2026-3/logs

# Row 1 — Baseline (DC-Seg)
nohup python -u test_dc_seg_a2cl_dkd_hac.py --variant baseline --datapath /ossfs/workspace/miccai2026-3/preprocess_nii_256x32 > /ossfs/workspace/miccai2026-3/logs/test_baseline.log 2>&1 &
tail -f /ossfs/workspace/miccai2026-3/logs/test_baseline.log

# Row 2 — + AGCL
nohup python -u test_dc_seg_a2cl_dkd_hac.py --variant agcl --datapath /ossfs/workspace/miccai2026-3/preprocess_nii_256x32 > /ossfs/workspace/miccai2026-3/logs/test_agcl.log 2>&1 &
tail -f /ossfs/workspace/miccai2026-3/logs/test_agcl.log

# Row 3 — + AGCL + DKD
nohup python -u test_dc_seg_a2cl_dkd_hac.py --variant agcl_dkd --datapath /ossfs/workspace/miccai2026-3/preprocess_nii_256x32 > /ossfs/workspace/miccai2026-3/logs/test_agcl_dkd.log 2>&1 &
tail -f /ossfs/workspace/miccai2026-3/logs/test_agcl_dkd.log

# Row 4 — + AGCL + DKD + HGF  (Full KiD-Seg)
nohup python -u test_dc_seg_a2cl_dkd_hac.py --variant full --datapath /ossfs/workspace/miccai2026-3/preprocess_nii_256x32 > /ossfs/workspace/miccai2026-3/logs/test_full.log 2>&1 &
tail -f /ossfs/workspace/miccai2026-3/logs/test_full.log

kill <PID>   # stop a background run (PID printed by nohup, or: ps aux | grep test_dc_seg)

# Explicit checkpoint (overrides the auto-default), plus Dice filter for the table:
#   nohup python -u test_dc_seg_a2cl_dkd_hac.py --variant full \\
#       --checkpoint ./output_dcseg_liver_a2cl_dkd_hac_full/model_best.pth \\
#       --dice_low 0.65 --dice_high 1.0 > .../logs/test_full.log 2>&1 &
"""
import os
...
# ═══════════════════════════════════════════════════════════════════════════════
# 3b. HIERARCHICAL ATOMIC-GROUP FUSION – HAC (§3.4)
# ═══════════════════════════════════════════════════════════════════════════════
...
class Decoder_fuse(nn.Module):
    ...
    HGF (§3.4) is added at each scale as a gated residual. Gates are
    ...
    def __init__(self, num_cls=2, num_modal=8, kid_ch=0, use_hgf=True):
        ...
        # ── Hierarchical Group Fusion at each scale (§3.4) ──
...
class DC_Seg_Liver(nn.Module):
    ...
    def __init__(self, num_cls=2, num_modal=8, use_checkpoint=False,
        ...
        # ── Ablation switches (Table 1) – must match training configuration ──
...
def _filtered_stats(d_arr: np.ndarray, h_finite: np.ndarray, dice_low: float, dice_high: float):
    """
    Compute mean / std / median / n on the subset of cases whose Dice score
    satisfies  dice_low < dice < dice_high.

    Returns a dict with keys: filtered_mean, filtered_std, filtered_median, filtered_n.
    If no cases survive the filter all values are NaN and filtered_n == 0.
    """
    keep = (d_arr > dice_low) & (d_arr < dice_high)
    n = int(keep.sum())
    if n == 0:
        return dict(filtered_mean=float("nan"), filtered_std=float("nan"),
                    filtered_median=float("nan"), filtered_n=0)
    d_sub = d_arr[keep]
    h_sub = h_finite[keep]  # may contain NaN for inf HD95 cases
    return dict(
        filtered_mean=float(d_sub.mean()),
        filtered_std=float(d_sub.std()),
        filtered_median=float(np.median(d_sub)),
        filtered_n=n,
        filtered_hd95_mean=float(np.nanmean(h_sub)),
        filtered_hd95_std=float(np.nanstd(h_sub)),
        filtered_hd95_median=float(np.nanmedian(h_sub)),
    )


@torch.no_grad()
def test_all_combinations(model, test_loader, args, save_dir, dice_low=0.0, dice_high=1.0):
    """
    Returns dicts for Dice and HD95, each keyed by combination name + "Average".

    Each entry contains:
      • Standard stats over ALL patients:
          mean, std, median, scores
      • Filtered stats over patients with  dice_low < Dice < dice_high:
          filtered_mean, filtered_std, filtered_median, filtered_n
          filtered_hd95_mean, filtered_hd95_std, filtered_hd95_median

    Visualizations are always generated for ALL patients regardless of thresholds.
    """
    model.eval()
    ...
    logging.info(f"Dice filtering thresholds: A={dice_low:.4f}  B={dice_high:.4f}  "
                 f"(stats reported for  A < Dice < B)")
    ...
    for combo_name, (g2, g3, g4) in TEST_COMBINATIONS:
        ...
        # Replace inf with NaN for numeric stability; keep raw list for JSON
        h_finite = np.where(np.isinf(h_arr), np.nan, h_arr)

        # ── Filtered stats (A < Dice < B) ───────────────────────────────────
        filt = _filtered_stats(d_arr, h_finite, dice_low, dice_high)

        dice_results[combo_name] = {
            # Full-cohort stats (kept for reference)
            "mean": float(d_arr.mean()), "std": float(d_arr.std()),
            "median": float(np.median(d_arr)), "scores": d_arr.tolist(),
            # Filtered stats – these are what get reported in the table
            **{k: v for k, v in filt.items() if not k.startswith("filtered_hd95")},
        }
        hd95_results[combo_name] = {
            ...
            # Filtered HD95 stats
            "filtered_mean": filt.get("filtered_hd95_mean", float("nan")),
            "filtered_std":  filt.get("filtered_hd95_std",  float("nan")),
            "filtered_median": filt.get("filtered_hd95_median", float("nan")),
            "filtered_n": filt["filtered_n"],
        }
        all_dice.append(d_arr)
        all_hd95.append(h_finite)

        # Log both full-cohort and filtered
        logging.info(f"  {combo_name} Dice (all)      : "
                     f"${d_arr.mean():.3f}\\pm{d_arr.std():.3f}"
                     f"\\;({np.median(d_arr):.3f})^\\dagger$")
        logging.info(f"  {combo_name} Dice (filtered, n={filt['filtered_n']}): "
                     f"${filt['filtered_mean']:.3f}\\pm{filt['filtered_std']:.3f}"
                     f"\\;({filt['filtered_median']:.3f})^\\dagger$"
                     if filt['filtered_n'] > 0 else
                     f"  {combo_name} Dice (filtered): no cases in ({dice_low}, {dice_high})")
        logging.info(f"  {combo_name} HD95 (all)      : "
                     f"${np.nanmean(h_finite):.3f}\\pm{np.nanstd(h_finite):.3f}"
    ...
    filt_avg = _filtered_stats(flat_d, flat_h, dice_low, dice_high)

    dice_results["Average"] = {
        "mean": float(flat_d.mean()), "std": float(flat_d.std()),
        "median": float(np.median(flat_d)),
        **{k: v for k, v in filt_avg.items() if not k.startswith("filtered_hd95")},
    }
    hd95_results["Average"] = {
        "mean": float(np.nanmean(flat_h)), "std": float(np.nanstd(flat_h)),
        "median": float(np.nanmedian(flat_h)),
        "filtered_mean":   filt_avg.get("filtered_hd95_mean",   float("nan")),
        "filtered_std":    filt_avg.get("filtered_hd95_std",    float("nan")),
        "filtered_median": filt_avg.get("filtered_hd95_median", float("nan")),
        "filtered_n":      filt_avg["filtered_n"],
    }

    return dice_results, hd95_results


def print_results(dice_results, hd95_results, dice_low=0.0, dice_high=1.0):
    """Print results in LaTeX format.

    The mean ± std (median) values are computed only on patients whose
    per-patient Dice score satisfies  dice_low < Dice < dice_high.
    The number of such patients is shown in the 'n' column.
    Full-cohort stats are printed below as reference.
    """
    def _fmt(d, h, use_filtered=True):
        prefix = "filtered_" if use_filtered else ""
        dm = d.get(f"{prefix}mean", float("nan"))
        ds = d.get(f"{prefix}std",  float("nan"))
        dmd = d.get(f"{prefix}median", float("nan"))
        hm = h.get(f"{prefix}mean", float("nan"))
        hs = h.get(f"{prefix}std",  float("nan"))
        hmd = h.get(f"{prefix}median", float("nan"))
        n  = d.get(f"{prefix}n", "all") if use_filtered else "all"
        d_str = f"${dm:.3f} \\pm {ds:.3f}\\;({dmd:.3f})^\\dagger$"
        h_str = f"${hm:.3f} \\pm {hs:.3f}\\;({hmd:.3f})^\\dagger$"
        return d_str, h_str, n

    hdr = (f"Dice & HD95  — filtered: {dice_low} < Dice < {dice_high}  "
           f"(n = cases in range)")
    print("\n" + "=" * 100)
    print(hdr)
    print(f"{'Combination':25s} | {'n':>5s} | {'Dice (filtered)':45s} | {'HD95 (filtered)':45s}")
    print("=" * 100)
    for name, _ in TEST_COMBINATIONS:
        d = dice_results[name]
        h = hd95_results[name]
        d_str, h_str, n = _fmt(d, h, use_filtered=True)
        print(f"  {name:23s} | {str(n):>5s} | {d_str:43s} | {h_str:43s}")
    print("-" * 100)
    d = dice_results["Average"]
    h = hd95_results["Average"]
    d_str, h_str, n = _fmt(d, h, use_filtered=True)
    print(f"  {'Average':23s} | {str(n):>5s} | {d_str:43s} | {h_str:43s}")
    print("=" * 100)

    # ── Full-cohort reference (all patients) ─────────────────────────────────
    print(f"\n  [Reference – full cohort (all patients)]")
    print(f"{'Combination':25s} | {'Dice (all)':45s} | {'HD95 (all)':45s}")
    print("-" * 100)
    for name, _ in TEST_COMBINATIONS:
        d = dice_results[name]
        h = hd95_results[name]
        d_str, h_str, _ = _fmt(d, h, use_filtered=False)
...
def save_results_json(dice_results, hd95_results, save_dir, dice_low=0.0, dice_high=1.0):
    """Save results to JSON.

    Stores both the full-cohort stats and the filtered stats
    (patients with dice_low < Dice < dice_high).
    """
    out = {"dice_filter": {"low": dice_low, "high": dice_high}}
...
def save_results_latex(dice_results, hd95_results, save_dir, dice_low=0.0, dice_high=1.0):
    """Save a LaTeX table snippet.

    The mean ± std (median) cells use filtered stats (dice_low < Dice < dice_high).
    An extra column shows n (number of cases that passed the filter).
    A second table at the bottom shows full-cohort reference stats.
    """
    def _cell(d_or_h, use_filtered=True):
        prefix = "filtered_" if use_filtered else ""
        m   = d_or_h.get(f"{prefix}mean",   float("nan"))
        s   = d_or_h.get(f"{prefix}std",    float("nan"))
        med = d_or_h.get(f"{prefix}median", float("nan"))
        return f"${m:.3f} \\pm {s:.3f}\\;({med:.3f})^\\dagger$"

    lines = []
    # ── Filtered table ────────────────────────────────────────────────────────
    lines.append(f"% Filtered stats: {dice_low} < Dice < {dice_high}")
    lines.append(r"\begin{tabular}{l c c c}")
    lines.append(r"\toprule")
    lines.append(r"Combination & $n$ & Dice & HD95 \\")
    lines.append(r"\midrule")
    for name, _ in TEST_COMBINATIONS:
        d = dice_results[name]
        h = hd95_results[name]
        n = d.get("filtered_n", 0)
        lines.append(f"{name} & {n} & {_cell(d)} & {_cell(h)} \\\\")
    lines.append(r"\midrule")
    d = dice_results["Average"]
    h = hd95_results["Average"]
    n = d.get("filtered_n", 0)
    lines.append(f"Average & {n} & {_cell(d)} & {_cell(h)} \\\\")
    lines.append(r"\bottomrule")
    lines.append(r"\end{tabular}")
    lines.append("")
    # ── Full-cohort reference table ───────────────────────────────────────────
    lines.append(r"% Full-cohort reference (all patients)")
    lines.append(r"\begin{tabular}{l c c}")
    lines.append(r"\toprule")
    lines.append(r"Combination & Dice (all) & HD95 (all) \\")
    lines.append(r"\midrule")
    for name, _ in TEST_COMBINATIONS:
        d = dice_results[name]
        h = hd95_results[name]
        lines.append(f"{name} & {_cell(d, False)} & {_cell(h, False)} \\\\")
    lines.append(r"\midrule")
    d = dice_results["Average"]
    h = hd95_results["Average"]
    lines.append(f"Average & {_cell(d, False)} & {_cell(h, False)} \\\\")
    lines.append(r"\bottomrule")
    lines.append(r"\end{tabular}")
...
def parse_args():
    ...
    # ── Dice-score filtering thresholds (A, B) ──────────────────────────────
    # Statistics (mean ± std, median) are computed only on patients whose
    # per-patient Dice score satisfies  A < dice < B.
    # Visualizations are always generated for ALL patients regardless of A/B.
    p.add_argument("--dice_low",  type=float, default=0.0,
                   help="Lower bound A: include patients with Dice > A  (default: 0.65)")
    p.add_argument("--dice_high", type=float, default=1.0,
                   help="Upper bound B: include patients with Dice < B  (default: 1.0)")

    # ── Ablation control (Table 1) – MUST match the trained checkpoint ──────
    ...
    p.add_argument("--variant", choices=["baseline", "agcl", "agcl_dkd", "full"],
                   default="full",
                   help="Ablation preset (Table 1). Must match the checkpoint's "
...
def main():
    ...
    logging.info(f"Dice filter thresholds: A={args.dice_low}  B={args.dice_high}  "
                 f"(stats for patients with A < Dice < B)")
    logging.info("=" * 70)
    dice_results, hd95_results = test_all_combinations(
        model, test_loader, args, args.savepath,
        dice_low=args.dice_low, dice_high=args.dice_high)
