...
if os.environ.get("KIDSEG_GPU_MEM_FRACTION"):  # fail fast (OOM) instead of paging GPU memory to host RAM
    torch.cuda.set_per_process_memory_fraction(float(os.environ["KIDSEG_GPU_MEM_FRACTION"]))
...
PATIENT_SPACING = {}   # patient -> (dx, dy, dz) in mm AFTER resizing (used for HD95 in mm)


def preprocess_patient(root, name, shape, use_cache=True):
    """Load + resize + min-max normalise one patient.

    The result is cached per target shape as float16 / uint8 ``.npy`` files under
    ``<root>/_cache_<X>x<Y>x<Z>/`` and memory-mapped on later runs, so the cohort is
    decoded from NIfTI only once and does not have to be held in RAM as float32.
    The physical voxel spacing of the resized grid is recorded in ``PATIENT_SPACING``.
    """
    pdir = Path(root) / name
    cdir = Path(root) / f"_cache_{shape[0]}x{shape[1]}x{shape[2]}"
    f_img, f_lbl, f_meta = cdir / f"{name}_img.npy", cdir / f"{name}_lbl.npy", cdir / f"{name}_meta.json"
    if use_cache and f_img.exists() and f_lbl.exists() and f_meta.exists():
        with open(f_meta) as fh:
            PATIENT_SPACING[name] = tuple(json.load(fh)["spacing_mm"])
        return np.load(f_img, mmap_mode="r"), np.load(f_lbl)
    imgs = []
    for suf in MODALITY_SUFFIXES:
        v = load_nifti(pdir / "images" / f"{name}_{suf}.nii.gz")
        v = normalize_volume(resize_volume(v, shape, order=1))
        imgs.append(v)
    images = np.stack(imgs, 0).astype(np.float16)
    lbl_nii = nib.load(str(pdir / "labels" / f"{name}_T2WI.nii.gz"))
    lbl = np.asarray(lbl_nii.dataobj, dtype=np.float32)
    zooms = [float(z) for z in lbl_nii.header.get_zooms()[:3]]
    spacing = tuple(z * s / t for z, s, t in zip(zooms, lbl.shape, shape))
    lbl = (resize_volume(lbl, shape, order=0) > 0.5).astype(np.uint8)
    PATIENT_SPACING[name] = spacing
    if use_cache:
        cdir.mkdir(parents=True, exist_ok=True)
        np.save(f_img, images)
        np.save(f_lbl, lbl)
        with open(f_meta, "w") as fh:
            json.dump({"spacing_mm": list(spacing), "shape": list(shape)}, fh)
        images = np.load(f_img, mmap_mode="r")
    return images, lbl
...
class LiverTestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return (torch.from_numpy(np.asarray(images, dtype=np.float32)),
    ...
    for combo_name, (g2, g3, g4) in TEST_COMBINATIONS:
        ...
        for x, y_int, names in test_loader:
            ...
            for b in range(x.size(0)):
                ...
                hd95_scores.append(compute_hd95(p_bin, g_bin, voxel_spacing=PATIENT_SPACING.get(names[b], (1.0, 1.0, 1.0))))
...
def generate_comprehensive_visualization(model, test_loader, args, save_dir,
    ...
    for x, y_int, names in test_loader:
        ...
        for b in range(x.size(0)):
            ...
            H, W, D = gt_pt.shape
            _ta = (gt_pt > 0).reshape(-1, D).sum(0)  # tumour area per axial slice
            z_mid = int(np.argmax(_ta)) if _ta.max() > 0 else D // 2
            modality_slices = []
            ...
            fig.suptitle(
                f"KiD-Seg [{args.variant}] Test Visualization — Patient: {name} — Axial Slice z={z_mid}\n"
...
def parse_args():
    ...
    p.add_argument("--max_vis_patients", type=int, default=0,

                   help="Limit the per-patient t-SNE + qualitative figures to the first N test "

                        "patients (0 = all). Metrics always use every test patient.")
...
def main():
    ...
    if _res.missing_keys or _res.unexpected_keys:
        raise RuntimeError("Checkpoint/architecture mismatch - missing: "
                           f"{_res.missing_keys[:8]} unexpected: {_res.unexpected_keys[:8]} "
                           "(does --variant match the trained checkpoint?)")
    ...
    # ══════════════════════════════════════════════════════════════════════
    logging.info("=" * 70)
    logging.info("EVALUATING ALL TEST COMBINATIONS ... (HD95 reported in mm using the resized voxel spacing)")
    ...
    # ══════════════════════════════════════════════════════════════════════
    vis_loader = test_loader  # optionally restrict the (slow) per-patient t-SNE + figures
    if getattr(args, "max_vis_patients", 0) and args.max_vis_patients < len(test_set):
        from torch.utils.data import Subset
        vis_loader = DataLoader(Subset(test_set, list(range(args.max_vis_patients))), batch_size=1,
                                num_workers=0, pin_memory=True)
        logging.info(f"t-SNE / figures limited to the first {args.max_vis_patients} test patients")
    ...
    fig_paths = generate_comprehensive_visualization(
        model, vis_loader, args, args.savepath,
